function holder = Adapt(filnamE,cv)
%% PROGRAM REMESH
...
np = single([]);
tg = single([]);
%tl = single(zeros(500,1));
tl = single([]);

%%%%%%%%%%% from findtin
%xt = single(zeros(2,1));
xt = single([]);
%arZ = single(zeros(3,1));
arZ = single([]);
...
%x = single(zeros(2, 500));
x = single([]);
%ibcs = single(zeros(10,30));
ibcs = single([]);
nonf = single([]);
nonr = single([]);
%iel = single(zeros(3,10000));
iel = single([]);
ti = single([]);
ts = single([]);
td = single([]);
tx = single([]);
%ti = single(zeros(1,1000));
ti = single(zeros(1,100000));
for ibs = 1:nbcs
    ...
    current = (ibs/nbcs)*100;
    fprintf('Current progress in interpolation %d \n', current);
end
...
%     counter = counter + 1;
    ...
    switch iwhat
        ...
        case 2  
            ...
            %nsmoo = temp(1);
            %nsmoo = single(nsmoo);
            %fprintf(' ENTER NUMBER OF SMOOTHING LOOPS %d\n',nsmoo)
